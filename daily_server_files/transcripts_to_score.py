...
def _parse_args():
    ...
    parser.add_argument('--csr_path', type=str, help='path to csr transcripts for scoring')
    parser.add_argument('--customer_path', type=str, help='path to customer transcripts for scoring')
...
def get_scores(dataloader, model, scoring_criterion):
    ...
    with torch.no_grad():
        ...
        df['id'] = id_arr
        df['CSR_Text'] = text_arr
        df['CSR_Text'] =df['CSR_Text'].apply(lambda x: '\n'.join(x))
        df['Importance Scores'] = attn_score_arr
    return df
...
if __name__ == "__main__":
    ...
    paths = [str(x) for x in Path(args.csr_path).glob("**/*.txt")]
    df = prepare_inference_df(args.csr_path)
    call_dataset = InferenceCallDataSet(df)
    with open(args.vocab_path, 'rb') as f:
        vocab = torch.load(f)
    c = Collate(vocab, args.device)
    call_dataloader = DataLoader(call_dataset, batch_size=1, shuffle=True,
